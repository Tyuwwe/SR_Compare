...
// Adapted from third_party/snapdragon-gsr/sgsr/v1/include/glsl/sgsr1_shader_mobile.frag:
//   * GLSL ES version raised to 310 (Vulkan SPIR-V requires ES >= 310)
//   * UseUniformBlock enabled (uniform block + explicit bindings for Vulkan)
//   * `int mode` made const and `highp` dropped from in-expression vec2
//     constructors (glslangValidator rejects both in Vulkan ES shaders)
